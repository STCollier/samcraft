...
static inline const uint64_t get_vertex(
	uint64_t x, 
	uint64_t y,
	uint64_t z,
	uint64_t u, 
	...
	return (type << 46) | (light << 41) | (opaque << 40) | (rotation << 35) | (face << 32) | (ao << 30) | (v << 24) | (u << 18) | ((z - 1) << 12) | ((y - 1) << 6) | (x - 1);
}
		...
		for (int forward = 1; forward < CS_P - 1; forward++) {
			...
			for (int right = 1; right < CS_P - 1; right++) {
				...
				while (copy_front) {
					...
					if (compare_forward(voxels, axis, forward, right, bit_pos, light_dir)) merged_forward[(right * CS_P) + bit_pos]++;
					else bits_merging_forward &= ~(1ULL << bit_pos);
				}
				...
				while (bits_stopped_forward) {
					...
					uint64_t type = arr_at(voxels, get_axis_i(axis, right, forward, bit_pos));
					uint64_t rotation = 0;
					uint64_t light = 31;

					uint64_t v1, v2, v3, v4;
					if (face == 0) {
						v1 = get_vertex(mesh_left, mesh_up, mesh_front, 0, mesh_back - mesh_front, ao_LF, face, rotation, opaque, light, type);
						v2 = get_vertex(mesh_left, mesh_up, mesh_back, 0, 0, ao_LB, face, rotation, opaque, light, type);
						v3 = get_vertex(mesh_right, mesh_up, mesh_back, mesh_right - mesh_left, 0, ao_RB, face, rotation, opaque, light, type);
						v4 = get_vertex(mesh_right, mesh_up, mesh_front, mesh_right - mesh_left, mesh_back - mesh_front, ao_RF, face, rotation, opaque, light, type);
					} else if (face == 1) {
						v1 = get_vertex(mesh_left, mesh_up, mesh_back, 0, 0, ao_LB, face, rotation, opaque, light, type);
						v2 = get_vertex(mesh_left, mesh_up, mesh_front, 0, mesh_back - mesh_front, ao_LF, face, rotation, opaque, light, type);
						v3 = get_vertex(mesh_right, mesh_up, mesh_front, mesh_right - mesh_left, mesh_back - mesh_front, ao_RF, face, rotation, opaque, light, type);
						v4 = get_vertex(mesh_right, mesh_up, mesh_back, mesh_right - mesh_left, 0, ao_RB, face, rotation, opaque, light, type);
					} else if (face == 2) {
						v1 = get_vertex(mesh_up, mesh_front, mesh_left, mesh_right - mesh_left, 0, ao_LF, face, rotation, opaque, light, type);
						v2 = get_vertex(mesh_up, mesh_back, mesh_left, mesh_right - mesh_left, mesh_back - mesh_front, ao_LB, face, rotation, opaque, light, type);
						v3 = get_vertex(mesh_up, mesh_back, mesh_right, 0, mesh_back - mesh_front, ao_RB, face, rotation, opaque, light, type);
						v4 = get_vertex(mesh_up, mesh_front, mesh_right, 0, 0, ao_RF, face, rotation, opaque, light, type);
					} else if (face == 3) {
						v1 = get_vertex(mesh_up, mesh_back, mesh_left, 0, mesh_back - mesh_front, ao_LB, face, rotation, opaque, light, type);
						v2 = get_vertex(mesh_up, mesh_front, mesh_left, 0, 0, ao_LF, face, rotation, opaque, light, type);
						v3 = get_vertex(mesh_up, mesh_front, mesh_right, mesh_right - mesh_left, 0, ao_RF, face, rotation, opaque, light, type);
						v4 = get_vertex(mesh_up, mesh_back, mesh_right, mesh_right - mesh_left, mesh_back - mesh_front, ao_RB, face, rotation, opaque, light, type);
					} else if (face == 4) {
						v1 = get_vertex(mesh_front, mesh_left, mesh_up, 0, 0, ao_LF, face, rotation, opaque, light, type);
						v2 = get_vertex(mesh_back, mesh_left, mesh_up, mesh_back - mesh_front, 0, ao_LB, face, rotation, opaque, light, type);
						v3 = get_vertex(mesh_back, mesh_right, mesh_up, mesh_back - mesh_front, mesh_right - mesh_left, ao_RB, face, rotation, opaque, light, type);
						v4 = get_vertex(mesh_front, mesh_right, mesh_up, 0, mesh_right - mesh_left, ao_RF, face, rotation, opaque, light, type);
					} else if (face == 5) {
						v1 = get_vertex(mesh_back, mesh_left, mesh_up, 0, 0, ao_LB, face, rotation, opaque, light, type);
						v2 = get_vertex(mesh_front, mesh_left, mesh_up, mesh_back - mesh_front, 0, ao_LF, face, rotation, opaque, light, type);
						v3 = get_vertex(mesh_front, mesh_right, mesh_up, mesh_back - mesh_front, mesh_right - mesh_left, ao_RF, face, rotation, opaque, light, type);
						v4 = get_vertex(mesh_back, mesh_right, mesh_up, 0, mesh_right - mesh_left,ao_RB, face, rotation, opaque, light, type);
					}
